  ...
  return (
    ...
    <div className="h-full min-h-0 w-full bg-[#020617] text-white flex flex-col overflow-hidden font-sans">
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        {/* LEFT PANE */}
        <section className="flex-[0.35] min-w-[400px] border-r border-white/5 flex flex-col bg-[#020617] z-10 min-h-0">
          
          <div className="h-12 shrink-0 border-b border-white/5 flex items-center justify-between px-6 bg-slate-950/30">
            <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Trade Setup</h2>
          ...
          <div className="flex-1 overflow-y-auto dark-scrollbar p-6 space-y-6 min-h-0 relative">
             ...
             <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Underlying Spot Price</label>
                  <Input type="number" step="0.01" className="h-8 bg-white/5 border-white/10 text-xs font-mono w-1/2" value={store.spotPrice} onChange={(e) => store.setSpotPrice(parseFloat(e.target.value) || 0)} />
                </div>
                ...
                <div className="space-y-3 pb-6"> 
                  <div className="flex items-center justify-between mb-2 border-b border-white/5 pb-2">
                    ...
                    <button className="p-1 hover:bg-white/10 rounded text-blue-400 transition-colors flex items-center text-[10px] font-bold uppercase tracking-wider" onClick={store.addLeg}>
                  ...
                  <div className="space-y-3"> 
                    {store.legs.map((leg, idx) => (
                      <div key={idx} className="bg-white/5 border border-white/10 p-3 rounded-md space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] font-mono font-bold text-slate-400">Leg {idx + 1}</span>
                          <button className="text-red-400/50 hover:text-red-400 transition-colors" onClick={() => store.removeLeg(idx)}><Minus className="w-3 h-3" /></button>
                        </div>
                        
                        <div className="grid grid-cols-4 gap-2">
                          <Select value={leg.action} onValueChange={(val: any) => store.updateLeg(idx, "action", val)}>
                            <SelectTrigger className={`h-7 text-xs font-bold uppercase border-none ${leg.action === 'buy' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}`}><SelectValue /></SelectTrigger>
                          ...
                          <Input type="number" placeholder="Qty" className="h-7 bg-black/20 border-white/5 text-xs font-mono" value={leg.quantity} onChange={(e) => store.updateLeg(idx, "quantity", parseFloat(e.target.value) || 0)} />
                          
                          <Select value={leg.type} onValueChange={(val: any) => store.updateLeg(idx, "type", val)}>
                            <SelectTrigger className="h-7 text-xs font-bold uppercase border-white/5 bg-black/20 text-white"><SelectValue /></SelectTrigger>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                             <label className="text-[8px] uppercase tracking-widest text-slate-500">Strike</label>
                             <Input type="number" step="0.5" className="h-7 bg-black/20 border-white/5 text-xs font-mono" value={leg.strike} onChange={(e) => store.updateLeg(idx, "strike", parseFloat(e.target.value) || 0)} />
                          </div>
                          <div className="space-y-1">
                             <label className="text-[8px] uppercase tracking-widest text-slate-500">Premium</label>
                             <Input type="number" step="0.01" className="h-7 bg-black/20 border-white/5 text-xs font-mono" value={leg.premium} onChange={(e) => store.updateLeg(idx, "premium", parseFloat(e.target.value) || 0)} />
                          </div>
                        </div>
                      </div>
                    ))}
                    {store.legs.length === 0 && (
                      <div className="text-center p-4 border border-dashed border-white/10 rounded-md text-slate-500 text-xs uppercase tracking-widest font-bold">
                        No legs added
                      </div>
                    )}
                  </div>
        ...
        <section className="flex-[0.65] flex flex-col bg-[#020617] relative z-0 min-h-0">
          <div className="h-12 shrink-0 border-b border-white/5 flex items-center px-6 bg-slate-950/30">
              <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Capital Analysis</h2>
          </div>

          {!store.result ? (
            <div className="flex-1 flex items-center justify-center text-slate-600 flex-col min-h-0">
              <ShieldAlert className="w-12 h-12 mb-4 opacity-20" />
              <p className="text-xs uppercase tracking-widest font-bold">Awaiting Setup</p>
            </div>
          ) : (
            <div className="flex-1 overflow-y-auto dark-scrollbar p-6 space-y-6 relative min-h-0">
              ...
              <div className="flex justify-between items-end border-b border-white/10 pb-4 relative z-10">
                <div>
                  <h2 className="text-2xl font-black tracking-tight text-white">{store.result.strategy_classification}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 relative z-10">
                <div className="bg-white/5 backdrop-blur-sm border border-red-500/20 rounded-lg p-4 shadow-lg">
                  <p className="text-[9px] uppercase tracking-widest text-red-400 font-bold mb-1">Total Margin Required</p>
                  <p className="text-2xl font-mono font-black text-white">${formatMetric(store.result.total_margin_req)}</p>
                </div>
                <div className={`bg-white/5 backdrop-blur-sm border rounded-lg p-4 shadow-lg ${store.result.net_premium > 0 ? 'border-emerald-500/20' : 'border-blue-500/20'}`}>
                  ...
                  <p className="text-2xl font-mono font-black text-white">${formatMetric(Math.abs(store.result.net_premium))}</p>
                </div>
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-4 shadow-lg">
                  ...
                  <p className="text-2xl font-mono font-black text-white">{formatMetric(store.result.max_return_on_capital, true)}</p>
                </div>
              </div>

              {/* Margin Breakdown Chart */}
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-5 shadow-xl relative z-10 mt-6">
                ...
                <div className="h-[250px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={marginChartData} layout="vertical" margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="rgba(255,255,255,0.05)" />
                      <XAxis type="number" fontSize={10} tickLine={false} axisLine={false} stroke="#64748b" tickFormatter={(val) => `$${val}`} />
                      <YAxis type="category" dataKey="name" fontSize={10} tickLine={false} axisLine={false} stroke="#64748b" width={50} />
                      <RechartsTooltip 
                      ...
                      />
                      <Bar dataKey="marginReq" radius={[0, 4, 4, 0]} barSize={30}>
