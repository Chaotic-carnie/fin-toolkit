...
export default function CapBudPage() {
  ...
  return (
    // 1. OUTermost shell: strict calc height and min-h-0 to prevent viewport overflow
    <div className="h-full min-h-0 w-full bg-[#020617] text-white flex flex-col overflow-hidden font-sans">
      
      {/* TITLE BAR (shrink-0 protects it from getting crushed) */}
      ...
      {/* 2. MAIN WORKSPACE - Crucial min-h-0 addition here */}
      <div className="flex-1 flex min-h-0 overflow-hidden">
        
        {/* LEFT PANE - Strict flex column with min-h-0 */}
        <section className="flex-[0.35] min-w-[350px] border-r border-white/5 flex flex-col bg-[#020617] z-10 min-h-0">
          ...
          {/* SCROLLABLE INPUTS - min-h-0 here enables the scrollbar! */}
          <div className="flex-1 overflow-y-auto dark-scrollbar p-6 space-y-6 min-h-0 relative">
             ...
             <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Project Name</label>
                      <Input className="h-8 bg-white/5 border-white/10 text-xs" value={store.projectName} onChange={(e) => store.setField("projectName", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Currency</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Discount Rate (Dec)</label>
                      <Input type="number" step="0.001" className="h-8 bg-white/5 border-white/10 text-xs font-mono" value={store.discountRate} onChange={(e) => store.setField("discountRate", parseFloat(e.target.value) || 0)} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Convention</label>
                ...
                <div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden">
                  <button onClick={() => setShowWacc(!showWacc)} className="w-full px-4 py-2 flex items-center justify-between bg-white/5 hover:bg-white/10 transition-colors">
                    <span className="text-[10px] uppercase tracking-widest text-slate-300 font-bold flex items-center gap-2">
                  ...
                  {showWacc && (
                    <div className="p-4 space-y-3 bg-black/20">
                      <div className="grid grid-cols-3 gap-2">
                        <div className="space-y-1"><label className="text-[9px] uppercase tracking-wider text-slate-500">Cost of Eq (Re)</label><Input type="number" step="0.01" className="h-7 text-xs bg-white/5 border-white/10 font-mono" value={store.waccRe} onChange={(e)=>store.setField("waccRe", +e.target.value)} /></div>
                        <div className="space-y-1"><label className="text-[9px] uppercase tracking-wider text-slate-500">Cost of Debt (Rd)</label><Input type="number" step="0.01" className="h-7 text-xs bg-white/5 border-white/10 font-mono" value={store.waccRd} onChange={(e)=>store.setField("waccRd", +e.target.value)} /></div>
                        <div className="space-y-1"><label className="text-[9px] uppercase tracking-wider text-slate-500">Tax Rate</label><Input type="number" step="0.01" className="h-7 text-xs bg-white/5 border-white/10 font-mono" value={store.waccTax} onChange={(e)=>store.setField("waccTax", +e.target.value)} /></div>
                        <div className="space-y-1"><label className="text-[9px] uppercase tracking-wider text-slate-500">Eq Weight (wE)</label><Input type="number" step="0.01" className="h-7 text-xs bg-white/5 border-white/10 font-mono" value={store.waccWe} onChange={(e)=>store.setField("waccWe", +e.target.value)} /></div>
                        <div className="space-y-1"><label className="text-[9px] uppercase tracking-wider text-slate-500">Debt Weight (wD)</label><Input type="number" step="0.01" className="h-7 text-xs bg-white/5 border-white/10 font-mono" value={store.waccWd} onChange={(e)=>store.setField("waccWd", +e.target.value)} /></div>
                      </div>
                      <div className="flex items-center gap-2 pt-1">
                        <Button size="sm" className="flex-1 h-7 text-[10px] bg-white/10 hover:bg-white/20 text-white" onClick={store.calculateWacc}>Compute</Button>
                        {store.computedWacc !== null && (
                          <Button size="sm" className="flex-1 h-7 text-[10px] bg-blue-600/20 text-blue-400 hover:bg-blue-600/40 border border-blue-500/30" onClick={store.applyWaccToDiscount}>Use: {(store.computedWacc * 100).toFixed(2)}%</Button>
                        )}
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-3 pb-6"> 
                  <div className="flex items-center justify-between mb-2 border-b border-white/5 pb-2">
                    ...
                    <div className="flex gap-1">
                      <button className="p-1 hover:bg-white/10 rounded text-slate-400 transition-colors" onClick={store.addYear}><Plus className="w-3 h-3" /></button>
                      <button className="p-1 hover:bg-white/10 rounded text-slate-400 transition-colors" onClick={store.removeYear}><Minus className="w-3 h-3" /></button>
                    </div>
                  </div>
                  
                  <div className="space-y-2"> 
                    {store.cashflows.map((cf, idx) => (
                      <div key={idx} className="flex items-center gap-3 bg-white/5 border border-white/5 p-1.5 rounded-md">
                        <div className={`w-12 text-center text-[10px] font-mono font-bold py-1 rounded ${idx === 0 ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                          t = {idx}
                        </div>
                        <Input 
                          type="number" step="0.01"
                          className="h-7 bg-transparent border-none text-right font-mono text-sm focus-visible:ring-0 px-2"
                          value={cf}
          ...
          {/* ACTION BAR - shrink-0 rigidly anchors this to the bottom of the section */}
          ...
          {!store.result ? (
            ...
            <div className="flex-1 overflow-y-auto dark-scrollbar p-6 space-y-6 relative min-h-0">
              ...
              <div className="flex justify-between items-end border-b border-white/10 pb-4 relative z-10">
                <div>
                  <h2 className="text-2xl font-black tracking-tight text-white">{store.result.project_name}</h2>
                  <p className="text-slate-500 text-[10px] uppercase tracking-widest mt-1 font-mono">Run: {store.result.run_id}</p>
                </div>
                <div className={`px-3 py-1 rounded border text-[11px] font-bold uppercase tracking-widest ${store.result.npv > 0 ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 relative z-10">
                {[
                  ...
                  <div key={i} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-3 shadow-lg">
                    ...
                    <p className={`text-lg font-mono font-bold ${kpi.color}`}>{kpi.val}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-4 border-b border-white/10 pt-2 relative z-10">
                {["overview", "table", "sensitivity"].map((tab) => (
                  <button 
                    ...
                    className={`pb-2 text-[10px] uppercase tracking-widest font-bold transition-colors border-b-2 ${activeTab === tab ? 'text-blue-400 border-blue-400' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
                  >
                    {tab}
                  </button>
                ))}
                ...
                <div className="space-y-6 relative z-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-5 shadow-xl">
                      <h3 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold flex items-center gap-2 mb-4">
                      ...
                      <div className="h-[250px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={npvProfileData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <RechartsTooltip cursor={{ strokeDasharray: "3 3", stroke: "rgba(255,255,255,0.2)" }} contentStyle={{ backgroundColor: "#0f172a", borderColor: "rgba(255,255,255,0.1)", borderRadius: "8px", fontSize: "12px", color: "#fff" }} />
                            <ReferenceLine y={0} stroke="rgba(255,255,255,0.2)" />
                  ...
                  {store.result.notes.length > 0 && (
                    <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                      <h4 className="text-[10px] uppercase tracking-widest text-amber-500 font-bold flex items-center mb-2">
                        <AlertCircle className="w-3 h-3 mr-2" /> Analytical Notes
                      </h4>
                      <ul className="list-disc pl-5 text-[11px] text-amber-500/80 space-y-1 font-mono">
              ...
              {/* TAB: TABLE */}
              {activeTab === "table" && (
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden shadow-xl relative z-10">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-white/5 border-b border-white/10 text-[9px] uppercase tracking-widest text-slate-400 font-bold">
              ...
              {activeTab === "sensitivity" && (
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden shadow-xl pb-4 relative z-10">
                  ...
                  <div className="overflow-x-auto p-4">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="text-[9px] uppercase tracking-widest text-slate-400 font-bold border-b border-white/10">
                          <th className="p-2 w-[120px]">CF Scale \ Rate</th>
